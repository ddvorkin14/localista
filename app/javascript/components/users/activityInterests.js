...
const ActivityInterests = ({ 
  ...
  return (
    <>
      <h2 className="text-lg font-semibold mb-4">Your activity interests</h2>
      {activityInterests.length > 0 && <hr/>}

      <div className="mt-2" style={{display: 'flex', flexWrap: 'wrap'}}>
        {activityInterests.map((interest) => {
          return (
            interest.kind === 'activity' && (
      ...
      <div>
        <label htmlFor="email" className="block text-sm font-medium text-gray-700 mb-1 mt-3">Activity interests</label>
        <input
          type="text"
